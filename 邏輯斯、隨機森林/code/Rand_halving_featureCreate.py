...
df['claim_per_income'] = df['claim_est_payout'] / df['annual_income']
df['vehicle_price_per_weight'] = df['vehicle_price'] / df['vehicle_weight']

X = df.drop(columns=['claim_number', 'fraud', 'living_status', 'high_education_ind',
                     'claim_est_payout', 'annual_income', 'vehicle_price', 'vehicle_weight'])  
y = df['fraud']
...
grid = list(ParameterGrid(param_grid))  # ✅ 產生參數組合的 dict 清單
